...
from models import Area, GameModel, SystemLog, User, log_system_event
...
def check_system_permission():
    return session.get("is_admin") and (
        session.get("is_master_admin")
        or session.get("can_manage_system")
        or session.get("can_manage_areas")
        or session.get("can_manage_games")
    )
...
def add_area():
    ...
    if "area_image" in request.files:
        ...
        if file and file.filename != "":
            ext = (
                file.filename.split(".")[-1].lower() if "." in file.filename else "png"
            )
...
def add_game_to_area(area_id):
    ...
    structured_checks = [
        {
            "name": c.strip(),
            "is_mandatory": bool(request.form.get(f"check_mandatory_{i}")),
        }
        for i, c in enumerate(check_names)
        if c.strip()
    ]
    ...
    if name:
        max_order = (
            db.session.query(db.func.max(GameModel.sort_order))
