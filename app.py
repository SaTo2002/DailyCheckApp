...
if not secret_key:
    raise RuntimeError(
        "❌ SECRET_KEY is missing from .env file! The app cannot start without it."
    )
...
with app.app_context():
    ...
    for col, default_val in [
        ...
        try:
            with db.engine.connect() as conn:
                conn.execute(
                    text(
                        f"ALTER TABLE users ADD COLUMN {col} TINYINT(1) DEFAULT {default_val}"
                    )
                )
                conn.commit()
        except Exception:
    ...
    for col_sql in [
        ...
        "ALTER TABLE daily_sessions ADD COLUMN negligence_reported TINYINT(1) DEFAULT 0",
    ]:
        try:
            with db.engine.connect() as conn:
                conn.execute(text(col_sql))
    ...
    try:
        with db.engine.connect() as conn:
            conn.execute(
                text("ALTER TABLE areas ADD COLUMN image VARCHAR(255) DEFAULT NULL")
            )
            conn.commit()
    except Exception:
        pass  # العمود موجود بالفعل
    ...
    if request.endpoint and 'static' not in request.endpoint:
        ...
        try:
            neglected = DailySession.query.filter(
                ...
                DailySession.negligence_reported == False
            ).all()
