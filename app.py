    ...
    if area_id:
        ...
        if game_id in game_ids:
            ...
            if current_index + 1 < len(game_ids):
                next_game_id = game_ids[current_index + 1]
...
def submit_report():
    ...
    for game_id in completed_games:
        ...
        new_report = GameReport(
            ...
            notes=notes, map_image_path=map_drawing,
    ...
    return "<div style='font-family: Arial; text-align: center; margin-top: 100px; direction: rtl;'><h1 style='color: #28a745;'>تم إرسال تقرير المنطقة بنجاح! 🎉</h1><br><a href='/' style='padding: 15px 30px; background-color: #007bff; color: white; text-decoration: none; border-radius: 5px; font-size: 18px;'>العودة للصفحة الرئيسية</a></div>"
...
def cancel_area():
    for game_id, data in session.get('game_data', {}).items():
...
def dashboard():
    ...
    for r in reports:
        ...
        # استرجاع بيانات اللعبة لتعيين الأسماء الصحيحة للفحوصات
        ...
        base_map = game_model.map_image if game_model and game_model.map_image else ""
        
        actual_check_names = []
        if game_model:
            try: actual_check_names = [c['name'] for c in json.loads(game_model.checks)]
            except: pass
        ...
        for k, v in checks.items():
            if k.startswith('check_'):
                try:
                    ...
                    name = actual_check_names[idx] if idx < len(actual_check_names) else k
                    mapped_checks[name] = 'OK' if v == 'سليم' else 'Not OK'
...
def manage_system():
    ...
    return render_template('manage_system.html', areas=areas, games=games)
...
def add_game():
    ...
    if name and area_id:
        new_game = GameModel(
            ...
            has_map=bool(request.form.get('has_map')), map_mandatory=bool(request.form.get('map_mandatory')),
            allow_photos=bool(request.form.get('allow_photos')), requires_photo=bool(request.form.get('requires_photo')),
            notes_mandatory=bool(request.form.get('notes_mandatory')), checks=json.dumps(structured_checks, ensure_ascii=False)
        )
...
def edit_game(game_id):
    ...
    if request.method == 'POST':
        ...
        check_names = request.form.getlist('check_names[]')
        structured_checks = [{"name": c.strip(), "is_mandatory": bool(request.form.get(f'check_mandatory_{i}'))} for i, c in enumerate(check_names) if c.strip()]
        game.checks = json.dumps(structured_checks, ensure_ascii=False)
        db.session.commit()
        return redirect(url_for('manage_system'))
        
    areas = Area.query.all()
    checks = json.loads(game.checks) if game.checks else []
    return render_template('edit_game.html', game=game, areas=areas, checks=checks)
...
def manage_users():
    ...
    if request.method == 'POST':
        ...
        if username and password:
            ...
            db.session.add(User(username=username, password_hash=hashed_pw, role=role))
            db.session.commit()
